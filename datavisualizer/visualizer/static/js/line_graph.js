document.getElementById('update-graph').addEventListener('click', function () {
    ...
    const title = document.getElementById('title_label').value.trim() || 'Line Graph';  // Default title if empty
    const xLabel = document.getElementById('x_axis_label').value.trim() || 'X-Axis';  // Default x-axis label if empty
    const yLabel = document.getElementById('y_axis_label').value.trim() || 'Y-Axis';  // Default y-axis label if empty
    ...
    fetch('/update_line_graph/', {
        ...
        .then(data => {
            ...
            document.getElementById('line-graph').src = 'data:image/png;base64,' + data.plot_image;
        })
...
document.getElementById('download-btn').addEventListener('click', function () {
    ...
    const title = document.getElementById('title_label').value || "line-graph"
